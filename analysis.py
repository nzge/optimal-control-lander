...
def regulation_metrics(x_hist, u_hist, t_grid, Q, R, Qf):
    """Summary metrics for a regulation rollout."""
    dt = np.diff(t_grid)
    state_norm = np.linalg.norm(x_hist, axis=1)
    weighted = np.array([weighted_state_norm(x_hist[k], Q) for k in range(len(t_grid))])

    u_norm = np.linalg.norm(u_hist, axis=1)
    settle_idx = np.where(state_norm < 0.05 * state_norm[0] + 1e-12)[0]
    settle_time = t_grid[settle_idx[0]] if settle_idx.size else np.nan

    return {
        "J": lqr.regulation_cost(x_hist, u_hist, Q, R, Qf, t_grid),
        "control_energy": control_energy(u_hist, t_grid),
...
def run_regulation(
    ...
    *,
    ...
    If lti is None, auto-detect from constant A at t=0 and t=tf.
    ...
    if lti:
        x_hist, u_hist = lqr.simulate_lti_closed_loop(A0, B0, t_grid, ctrl, x0)
    else:
        x_hist, u_hist = lqr.simulate_ltv_closed_loop(dynamics_func, t_grid, ctrl, x0, params)

    metrics = regulation_metrics(x_hist, u_hist, t_grid, Q, R, Qf)
    return {
        "x_hist": x_hist,
        "u_hist": u_hist,
        "P_interp": P_interp,
