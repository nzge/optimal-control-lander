...
CTRL_LABELS = [r"\delta T", r"\tau"]
CONTROLLABILITY_BLOCK_LABELS = ["B", "AB", r"A^2B", r"A^3B", r"A^4B", r"A^5B", r"A^6B"]


def controllability_snapshot(dynamics_func, t, params, *, mode_name=None):
    """Algebraic controllability matrix C = [B, AB, ..., A^{n-1}B] at trim time t."""
    A, B = dynamics_func(t, params)
    C = controllability_matrix(A, B)
    singulars = singular_values(C)
    return {
        "t": float(t),
        "mode_name": mode_name,
        "A": A,
        ...
        "sigma_min_controllable": sigma_min_controllable(singulars),
        "sigma_structural_min": sigma_structural_min(singulars),
    }


def compare_controllability_snapshots(params, times=None):
    """
    Hover (LTI) vs descent (LTV) algebraic controllability at selected trim times.

    Returns a list of records with full 7x14 matrices for side-by-side comparison.
    """
    if times is None:
        times = (0.0, 0.75 * params["tf_descent"])
    ...
    for t in times:
        rows.append(
            {
                "t": float(t),
                "descent_trim": descent_trim(t, params),
                "hover": controllability_snapshot(
                    get_hover_dynamics, t, params, mode_name="hover"
                ),
                "descent": controllability_snapshot(
                    get_descent_dynamics, t, params, mode_name="descent"
                ),
            }
        )
    return rows
...
def _latex_number(x, precision=3):
    if abs(x) < 10 ** -(precision + 1):
        return "0"
...
def matrix_to_latex(
    ...
    *,
    ...
    """
    Render a matrix as a LaTeX bmatrix.

    column_groups: optional list of (group_name, width) for \\multicolumn headers.
    """
    ...
    lines = []
    if column_groups:
        specs = "l" + "".join(f"c" * width for _, width in column_groups)
        lines.append(r"\begin{array}{" + specs + "}")
        header = [""]
        for name, width in column_groups:
            header.append(rf"\multicolumn{{{width}}}{{c}}{{{name}}}")
    ...
    for i in range(n_rows):
        cells = [_latex_number(M[i, j], precision) for j in range(n_cols)]
        if column_groups:
            row_label = row_labels[i] if row_labels else str(i)
            lines.append(" & ".join([row_label] + cells) + (r" \\" if i < n_rows - 1 else ""))
        else:
            lines.append(" & ".join(cells) + (r" \\" if i < n_rows - 1 else ""))

    lines.append(r"\end{array}" if column_groups else r"\end{bmatrix}")
    return "\n".join(lines)


def controllability_matrix_latex(snapshot, *, precision=3, resize=True):
    """LaTeX for one controllability matrix with grouped input columns."""
    n_inputs = snapshot["B"].shape[1]
    block_groups = [(label, n_inputs) for label in CONTROLLABILITY_BLOCK_LABELS]
    body = matrix_to_latex(
        snapshot["C"],
        row_labels=STATE_LABELS,
        precision=precision,
        column_groups=block_groups,
    )
    mode = snapshot.get("mode_name") or "system"
    t = snapshot["t"]
    caption = (
        rf"Controllability matrix $\mathcal{{C}}=[B,AB,\ldots,A^{{6}}B]$ "
        rf"({mode}, $t={t:.1f}\,\mathrm{{s}}$)"
    )
    wrapped = (
        rf"\resizebox{{\textwidth}}{{!}}{{$\displaystyle {body}$}}"
        if resize
        else f"$\\displaystyle {body}$"
    )
    return f"% {caption}\n{wrapped}"


def controllability_latex_report(params, times=None, *, precision=3):
    """
    Build a LaTeX fragment comparing hover vs descent controllability matrices.

    Includes rank / singular-value summary tables plus full grouped matrices.
    """
    rows = compare_controllability_snapshots(params, times=times)
    parts = [
        "% Auto-generated by dynamics.controllability_latex_report",
        r"\section*{Algebraic controllability: hover vs descent}",
        (
            r"State order: $x=[p_x,p_z,v_x,v_z,\theta,\omega,m]^\top$; "
            r"controls $u=[\delta T,\tau]^\top$; "
            r"$\mathcal{C}=[B,AB,\ldots,A^{6}B]\in\mathbb{R}^{7\times 14}$."
        ),
        "",
    ]
    ...
    for row in rows:
        t = row["t"]
        trim = row["descent_trim"]
        parts.append(rf"\subsection*{{Trim snapshot $t={t:.1f}\,\mathrm{{s}}$}}")
        parts.append(
            rf"Descent trim: $m={trim['m']:.2f}\,\mathrm{{kg}}$, "
            rf"$v_x^\ast={trim['vx']:.1f}\,\mathrm{{m/s}}$, "
            rf"$v_z^\ast={trim['vz']:.1f}\,\mathrm{{m/s}}$."
        )
        parts.append("")
        parts.append(r"\begin{table}[htbp]")
        parts.append(r"\centering")
        parts.append(r"\caption{Controllability rank and singular values}")
        parts.append(r"\begin{tabular}{@{}lcccc@{}}")
        parts.append(r"\toprule")
        parts.append(
            r"Model & $\mathrm{rank}(\mathcal{C})$ & $\sigma_1$ & $\sigma_6$ & $\sigma_7$ \\"
        )
        parts.append(r"\midrule")
        for key, label in (("hover", "Hover LTI"), ("descent", "Descent LTV")):
            snap = row[key]
            s = snap["singular_values"]
            parts.append(
                rf"{label} & {snap['rank']}/7 & {_latex_number(s[0], precision)} "
                rf"& {_latex_number(s[-2], precision)} & {_latex_number(s[-1], precision)} \\"
            )
        ...
        for key, label in (("hover", "Hover LTI"), ("descent", "Descent LTV")):
            snap = row[key]
            parts.append(rf"\paragraph{{{label} at $t={t:.1f}\,\mathrm{{s}}$.}}")
            parts.append(controllability_matrix_latex(snap, precision=precision))
            parts.append("")

    return "\n".join(parts)


def print_controllability_comparison(params, times=None):
    """Notebook-friendly summary of hover vs descent controllability matrices."""
    rows = compare_controllability_snapshots(params, times=times)
    for row in rows:
        t = row["t"]
        trim = row["descent_trim"]
        print(f"\n=== Controllability snapshot t={t:.1f}s ===")
        print(
            f"Descent trim: m={trim['m']:.2f} kg, "
            f"v_x*={trim['vx']:.1f} m/s, v_z*={trim['vz']:.1f} m/s"
        )
        for key in ("hover", "descent"):
            snap = row[key]
            print(f"\n{key}:")
            print(f"  C shape {snap['C'].shape}, rank {snap['rank']}/7")
            print(
                "  singular values:",
                np.array2string(snap["singular_values"], precision=4, suppress_small=True),
            )
    return rows
